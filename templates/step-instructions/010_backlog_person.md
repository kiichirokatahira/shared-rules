# Step 010 — 人間: 変更依頼の作成

このステップは**人間が対応**します。
変更依頼を作成し、Claude の **Planning Mode** で内容を精査してから提出します。

## 対応手順

1. `change-requests/010_backlog_person/` に変更依頼フォルダを作成する
   - フォルダ名: `YYYYMMDD-xxxx`（例: `20260604-add-user-auth`）
   - フォルダ内に `ChangeRequest.md` を作成する（テンプレート: `ChangeRequest.md` を参照）

   ```
   010_backlog_person/
     20260604-add-user-auth/     ← 変更依頼フォルダ
       ChangeRequest.md           ← 依頼書（テンプレートをコピーして記入）
   ```

2. Claude を **Planning Mode**（`/plan` コマンド）で起動し、以下を依頼する:

   ```
   @change-requests/010_backlog_person/20260604-add-user-auth/ChangeRequest.md
   この変更依頼のドラフトを確認して、依頼の目的・変更内容・完了条件が明確かどうか
   指摘してください。不明瞭な箇所があれば質問してください。
   ```

3. Claude の指摘・質問に回答し、`ChangeRequest.md` を修正する
4. Claude が「内容が明確」と判断したら、フォルダごと `020_planning_ai/` に移動する

   ```
   変更前: 010_backlog_person/20260604-add-user-auth/
   変更後: 020_planning_ai/20260604-add-user-auth/
   ```

## 備考

- Planning Mode の目的は「仕様の曖昧さを事前に潰すこと」です
- Claude が疑問を持たない状態にしてから 020 に送ることで、020→010 の往復を減らせます
- 「調査が必要」な項目は調査が必要と明記した上で提出してかまいません

## 1件ずつ処理について

このリポジトリでは**変更依頼を1件ずつ**処理します。

- `020_planning_ai/` 〜 `110_pr_ai/` のいずれかに変更依頼フォルダがある場合、次の変更依頼の処理は開始しません
- `watch-change-requests.py` が自動で待機を管理します。処理中の変更依頼が完了すると次を自動起動します
- 複数の変更依頼を同時に `020_planning_ai/` に移動しないでください（一方が自動待機になります）
